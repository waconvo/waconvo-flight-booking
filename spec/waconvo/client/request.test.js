import Request from '../../../src/waconvo/client/request';

let host = 'test.api.waconvo.com';
let port = 443;
let verb = 'GET';
let ssl = true;
let path = '/foo/bar';
  ...
  describe('.instance', () => {
    ...
    it('should store the instance variables', () => {
      ...
      expect(request.headers).toEqual({
        'Accept': 'application/json, application/vnd.waconvo+json',
        'User-Agent': 'waconvo-flight-booking/1.2.3 node/2.3.4 waconvo-cli/3.4.5',
        'Authorization': 'Bearer token',
        'Content-Type': 'application/vnd.waconvo+json'
      });
    });
    ...
    describe('.userAgent', () => {
      it('should build the user agent', () => {
        ...
        expect(request.userAgent()).toBe('waconvo-flight-booking/1.2.3 node/2.3.4');
      });

      it('should build use the custom App ID and Version when present', () => {
        request.appId = 'waconvo-cli';
        request.appVersion = '3.4.5';
        expect(request.userAgent()).toBe('waconvo-flight-booking/1.2.3 node/2.3.4 waconvo-cli/3.4.5');
      });
    });
    ...
    describe('.addContentTypeHeader', () => {
      ...
      it('skip if the verb is not POST', () => {
        ...
        request.headers = {};
        request.addContentTypeHeader();
        expect(request.headers['Content-Type']).toBe('application/vnd.waconvo+json');
      });
    });
